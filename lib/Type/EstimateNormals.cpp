...
EstimatePerPointCovariances(const MyType::PointCloud &pointCloud) {
    const auto &points = pointCloud.points;
    std::vector<Eigen::Matrix3d> covariances;
    covariances.resize(points.size());

    auto const kdTree = KdTree<MyType::PointCloud>(pointCloud);

#pragma omp parallel for schedule(static)
    for (int i = 0; i < (int)points.size(); i++) {
        auto const indices = kdTree.knnSearch(points[i], 5);
        covariances[i] = ComputeCovariance(points, indices);
    }
    return covariances;
}
...
void PointCloud::estimateNormals() {
    ...
    bool has_normal = (normals.size() == points.size());
    if (!has_normal) {
        normals.resize(points.size());
    }
    std::vector<Eigen::Matrix3d> covariances;
    covariances = EstimatePerPointCovariances(*this);
...
#pragma omp parallel
    for (int i = 0; i < (int)covariances.size(); i++) {
        ...
        if (has_normal && normal.dot(normals[i]) < 0.0) {
            normal *= -1.0;
        }
        normals[i] = normal;
    }
